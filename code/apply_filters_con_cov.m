function output = apply_filters_con_cov(paths_array, cov_array)
    % This function removes the entries (rows) of the paths_array and cov_array
    % that contain missing data in either of the arrays. This function is very 
    % similar to apply_filters_con_cov_2.m, but checks for missing data in rows.
    ...
    if isempty(cov_array)
        missing_data = any(cellfun(@isempty, paths_array), 2);
        ...
        if size(paths_array, 1) ~= size(cov_array, 1)
            error('The paths array and the covariates array are of different lengths.');
        end
        % Find indices of missing data
        missing_data = any(cellfun(@isempty, paths_array), 2) | any(cellfun(@isempty, cov_array), 2) | any(cellfun(@(x) any(isnan(x)), cov_array), 2);
        paths_array_filtered = paths_array(~missing_data,:);
