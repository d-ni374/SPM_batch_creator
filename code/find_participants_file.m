function participants_file_path = find_participants_file(nodes, node_idx, task, factorial_design_idx)
    ...
    if isfield(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input, 'Covariates')
        participants_file_path = cell(1, length(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates));
        for j = 1:length(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates)
            if iscell(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates) && isfield(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates{j}, 'Name') && ~isempty(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates{j}.Name)
                participants_info_files = fct_lib.get_file_names(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.MainProjectFolder, nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates{j}.CovariatesFileRegexp);
                if isempty(participants_info_files)
                    warning('No participants file found in the main project folder for covariate %s.', nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates{j}.Name);
                    participants_info_files = fct_lib.recursdir(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.MainProjectFolder, nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates{j}.CovariatesFileRegexp);
                    if isempty(participants_info_files)
                        warning('No participants file found for covariates %s. Covariate %s will be skipped!', nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates{j}.Name);
                        return;
                    elseif length(participants_info_files) > 1
                        ...
                        if ~isempty(participants_info_files_filtered)
                            if length(participants_info_files_filtered) > 1
                                ...
                                warning('Multiple participants files found for covariates %s. Using the first one...', nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates{j}.Name);
                            else
                ...
                elseif length(participants_info_files) > 1
                    ...
                    if ~isempty(participants_info_files_filtered)
                        ...
                        warning('Multiple participants files found in the main project folder for covariate %s. Using the first one...', nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.Covariates{j}.Name);
                    end
                else
                    participants_info_files = fullfile(nodes{node_idx}.Transformations.Instructions{factorial_design_idx}.Input.MainProjectFolder, participants_info_files); % full path needed
                end
